---
title: "ISA Inclusion Rules: The lNewIntr Module"
output: 
  rmarkdown::html_vignette:
    toc: true
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{ISA Inclusion Rules: The lNewIntr Module}
...
The **`lNewIntr`** module controls these inclusion rules, determining the dynamic flow of treatments through your platform trial.
...
For common scenarios, use the helper function `lNewIntr()`:
...
- **Pattern**: ISAs enter, run for their duration, then finish, making room for new ones
...
**1. `$fnNewIntr`** - A function that decides how many new ISAs to add at the current time

- **Input**: `lPltfTrial` (current trial status) and `lAddArgs` (your custom parameters)
...
**Returns**: An `lNewIntr` object that replaces finished ISAs with new ones
